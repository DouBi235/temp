...
Page({
  ...
  tempSubmit: function (e) {
    ...
    if (data.region[0] == data.region[1] || data.region[1] == '县') {
      form.str_region = data.region[0]
    } else {
      form.str_region = data.region[0] + data.region[1]
    }
    form.begin_place = form.begin_place.replace(/\s+/g, "")
    if (!form.active_name || !form.active_route || !form.begin_place || !form.people_num || !data.optime || !data.edtime || !data.bmtime || !data.cartype || !form.str_region) {
      wx.showModal({
        title: '提示',
    ...
    var totalplace = form.str_region + form.begin_place
    var obj = {
